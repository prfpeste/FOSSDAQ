...
#define N 3                 // Enter the number of settings here. A setting can have many possible arguments (e.g., setting0=shape, setting1=color).
#define noSettings 0        // If no settings are required, set 'noSettings' = 1. If settings are required, set 'noSettings' = 0.

String ID="ao-3x-0to10V_";  // ID prefex of the specific Arduino. The '_' at the end is necessary!
...
#include <SPI.h>
#include <EEPROM.h>
static const int SHDN_PIN = 12;
static const int LDAC_PIN = 9;
...
switch (initStatus) {
  ...
  case 3:
    ...
    digitalWrite(SHDN_PIN, HIGH); // Hardware-Shutdown-Pin deaktivieren (DAC an)

    pinMode(LDAC_PIN, OUTPUT);
    digitalWrite(LDAC_PIN, LOW);  // LDAC dauerhaft LOW -> sofortige Ausgabe
    //=====================================================================================================================================================================    
    break;
...
void writeToDAC(uint16_t value, int CS_PIN = 2) {
  value &= 0x0FFF; // nur 12 Bit Nutzdaten

  // Bit15=0 (Kanal, egal), Bit14=1 (BUF), Bit13=1 (Gain 1x), Bit12=1 (aktiv)
  uint16_t command = 0x7000 | value;
...
void controll() {                                                         // function to control outputs
  ...
  if (startInit(received)) return;
    ...
    if (command < 0) {
      Serial.println("ERROR: invalid voltage (< 0 mV)"); 
      errorStatus();
      return;
    }
    if (command > 10000) {
      Serial.println("ERROR: invalid voltage (> 10000 mV)"); 
      errorStatus();
      return;
    }

    uint16_t voltageInSteps = (uint16_t) round( (float) command / 2.5 );
