...
SSID="MeinHotspot"
PASSWORD="MeinPasswort123"      # mind. 8 Zeichen
...
PORTAL_TARGET="http://192.168.50.1:5000"        # Ziel-Webseite, die beim Verbinden geöffnet wird
CAPTIVE_API_PATH="/captive-portal-api"          # Pfad, unter dem die RFC8908-JSON-API liegt
# ---------------------------------------

# Von install.sh bei der Installation abgefragt (SSID/Passwort) und hier
# hinterlegt - überschreibt die Standardwerte oben. Existiert die Datei
# nicht (z.B. beim direkten Testen dieses Skripts ohne install.sh), gelten
# einfach die Standardwerte oben weiter.
...
CAPTIVE_API_URL="http://$HOTSPOT_IP$CAPTIVE_API_PATH"
...
NGINX
    ...
    echo ">>> Fertig installiert. nginx leitet Port 80 an $PORTAL_TARGET weiter."
    echo ">>> Captive-Portal-API erreichbar unter: $CAPTIVE_API_URL"
    echo ">>> Stelle sicher, dass dein Dienst (z.B. Node-RED) auf Port 1880 läuft."
...
# DNS-Wildcard: jede Domain wird auf uns selbst aufgelöst (Fallback für
# Geräte/OS ohne RFC-8910-Unterstützung, z.B. Windows)
address=/#/$HOTSPOT_IP
no-resolv
no-poll
DNSMASQ
...
case "$1" in
    ...
    stop) stop_hotspot ;;
    *)
        echo "Verwendung: sudo $0 {install|start|stop}"
        exit 1
        ;;
esac
